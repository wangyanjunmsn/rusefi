...
float TriggerStateWithRunningStatistics::calculateInstantRpm(int *prevIndex, efitime_t nowNt DECLARE_ENGINE_PARAMETER_SUFFIX) {
	...
	*prevIndex = TRIGGER_SHAPE(triggerIndexByAngle[(int)previousAngle]);
	...
	efitime_t timeOfPreviousAngleEvent = timeOfLastEvent[*prevIndex];
	if (timeOfPreviousAngleEvent == 0) {
		// that previous angle event has not ever happened yet
		return prevInstantRpmValue;
	}
	uint32_t time = nowNt - timeOfPreviousAngleEvent;
